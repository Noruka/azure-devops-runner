...
AGENT_PACKAGE_URL=$(curl -fsSL -u "user:$(cat /azp/.token)" -H "Accept: application/json" "${AZP_URL}/_apis/distributedtask/packages/agent?platform=linux-x64&%24top=1" jq -r '.value[0].downloadUrl')
...
/azp/config.sh --unattended --agent "${AZP_AGENT_NAME:-$(hostname)}" --url "$AZP_URL" --auth PAT --token "$(cat /azp/.token)" --pool "$AZP_POOL" --work "${AZP_WORK:-_work}" --acceptTeeEula
